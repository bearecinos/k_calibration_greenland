...
# model Frontal ablation estimates, within RACMO Frontal
# ablation estimates range.
import os
import sys
import numpy as np
import pandas as pd
...
d_racmo = pd.read_csv(os.path.join(MAIN_PATH, config['processed_racmo']))

# Path to write out the data:
output_path = os.path.join(MAIN_PATH, config['racmo_calibration_results'])
...
for j, f in enumerate(filenames):
    ...
    if glacier.empty:
        ...
        index = d_racmo.index[d_racmo['RGI_ID'] == rgi_id].tolist()

        if len(index) == 0:
            print('There is no Racmo data for this glacier' + rgi_id)
            files_no_racmo_data = np.append(files_no_racmo_data, rgi_id)
            area = rgidf.Area.loc[rgidf.RGIId == rgi_id].values
